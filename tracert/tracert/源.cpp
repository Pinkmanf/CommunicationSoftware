...
int main(int argc,char* argv[])
{
    ...
    {
        printf("Failed to initialize the WinSock2 DLL.error code:%d\r\n", WSAGetLastError());
        return -1;
    }
    //得到IP地址
    ...
    cout << "Tracing route to " << argv[1] <<" [" << inet_ntoa(*(in_addr*)(&ulDestIP)) <<"]" << " with a maximum of 30 hops.\n" << endl;
    cout << "====开始跟踪“目的IP地址/域名”(最大30跳 )====:" << endl;
    ...
    {
        printf("Failed to set recv timeout,error code:%d", WSAGetLastError());
        closesocket(sockRaw);
        WSACleanup();
        return -1;
    }
    ...
    {
        printf("Failed to set send timeout,error code:%d", WSAGetLastError());
        closesocket(sockRaw);
        WSACleanup();
        return -1;
    }
